function Step6AthruC2(sourceFolder, mainDestFolders, baseDestFolder, destSubFolder, plotBool6A, plotBool6B, plotBool6C, upperThreshold, zScoreLimit, flatStdThresh, removeDiags, removeNos)
...
%function Step6AthruC(sourceFolder, destFolder, destSubFolder, plotBool, upperThreshold, zScoreLimit, flatStdThresh)
    ...
    for j = 1:length(files)
        ...
        for i = 1:length(condNames)
            ...
            for j2 = 1:length(triggerNames)
                ...
                blockTotData = zeros(61,1051);
                blockTotChannels = zeros(61,1);
                ...
                for k = 1:length(blockNames)
                    ...
                    epTotData = zeros(61,1051);
                    epTotChannels = zeros(61,1);
                    ...
                    for m = 1:length(epochNames)                      
                        ...
                        nRows = size(matrix, 1);
                        if nRows == 63
                            reducedChannels1 = matrix;
                        elseif nRows == 70 || nRows == 88
                            rowsToDelete = [32, 65:nRows];
                            reducedChannels1 = matrix(setdiff(1:nRows, rowsToDelete), :);
                        elseif nRows == 69
                        ...
                        nRows2 = size(reducedChannels1, 1);
                        rowsToDelete2 = [13,19];
                        reducedChannels = reducedChannels1(setdiff(1:nRows2, rowsToDelete2), :);
                        ...
                        numBad = 61 - numGood; tempChannCounts(10) = tempChannCounts(10) + numBad;
                        
                        % Bucket 10 = extra zeroed channels caused by the wipe (post - pre)
                        if majorityWipe
                            extraZeroed = numBad - pre_numBad; % equals 61 - pre_numBad
                        ...
                        if plotBool6A
                            ...
                            numPk2PkBad = 61 - sum(passMatrixDiff(:));
    ...
    disp(['Percent correct: ', num2str(round(chanPerc,2)), '%']);
    totalPerc = (totGoodChanns / (totGoodChanns + totBadChanns)) * 100;
    subjChanStats('Total') = [totGoodChanns, totBadChanns, totalPerc];
    fprintf('\n');
    ...
    for i = 1:length(responseKeys)
        key  = responseKeys{i};
        vals = subjRespStats(key);   % <-- FIX: use the response map
