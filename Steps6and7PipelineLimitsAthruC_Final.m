...
folderName = 'Thresh_100_RD_RMasts';
...
for i = 1:length(subjSettings)     
    ...
    %Step6AthruC(step6AInDir, Step6OutDirs, step6ACBaseDir, fullfile(subjType, subjSetting), graphBool6A, graphBool6B, graphBool6C, upperThreshold, zScoreLimit, flatStdThresh, removeDiags, removeNos);
    Step6AthruC2(step6AInDir, Step6OutDirs, step6ACBaseDir, fullfile(subjType, subjSetting), graphBool6A, graphBool6B, graphBool6C, upperThreshold, zScoreLimit, flatStdThresh, removeDiags, removeNos);
    ...
    %{
    %% Step 6C2a - Filter Triggers (Yes only)
    disp('Starting Step 6C2a - Filter Triggers:'); tic;
    Step6CNew2FilterTriggers(step6COutDir, step6C2aOutDir)
    disp(['Step 6C2a - Filter Triggers completed in ', num2str(toc/60), ' minutes.']);
    %}

     %% Step 6C2b - Combine Blocks
    disp('Starting Step 6C2b - Combine Blocks:'); tic;
    ...
    Step7GraphAll3(step6GOutDir, step7AOutDir, step7GTitle, maxTime, upBound, lowBound);
...
Step7CompareGroupsSE(dirList, dirNames, [1, 4; 2, 3], step7CompOutDir, maxTime)
